...
export const ForgotPassword = TryCatch(async (req, res, next) => {
    ...
    const resetToken = jwt.sign({ email: requiredUser.email, type: "reset" },
...
export const ResetPassword = TryCatch(async (req, res, next) => {
    ...
    let decode: any
    try {
        if (!token) throw new ErrorHandler('Token is missing', 400);
        decode = jwt.verify(token as string, process.env.JWT_SECRET as string);
        console.log(decode);
    }
    catch (err) {
        console.log(err);
