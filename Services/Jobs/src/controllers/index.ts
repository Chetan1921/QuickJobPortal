...
export const DeleteCompany = TryCatch(async (req: AuthenticatedRequest, res, next) => {
    ...
    if (user.role !== "recruiter") {
        throw new ErrorHandler(
            "You are not authorised to create jobs",
            403
        );
    }
...
export const CreateJob = TryCatch(
    ...
    `;
        ...
        if (!company) {
            throw new ErrorHandler(
                "Company not found or you are not authorised",
                404
            );
        }
...
export const UpdateJob = TryCatch(
    async (req: AuthenticatedRequest, res, next) => {
        ...
        if (user.role !== "recruiter") {
            throw new ErrorHandler(
                "You are not authorised to update jobs",
                403
            );
        }
    ...
    `;
        ...
        if (!job) {
            throw new ErrorHandler(
                "Job not found or you are not authorised",
                404
            );
        }
...
export const GetApplicationsByJob = TryCatch(
    async (req: AuthenticatedRequest, res) => {
        const user = req.user;
