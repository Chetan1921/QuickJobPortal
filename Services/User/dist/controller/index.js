...
import { GetCompleteUserDetailbyUserId, UpdateProfile, UpdateProfilePicture, AddSkillToUser, UpdateResume } from "../utils/db.js";
...
let sql;
export const myprofileController = TryCatch(async (req, res, next) => {
    const user = req.user;
...
export const GetUserProfileByIdController = TryCatch(async (req, res, next) => {
...
export const UpdateUserProfileController = TryCatch(async (req, res, next) => {
    const user = req.user;
    if (!user) {
...
export const UpdateProfilePicController = TryCatch(async (req, res, next) => {
    const user = req.user;
    if (!user) {
        throw new ErrorHandler("Authentication is Required", 400);
    }
...
export const UpdateResumeController = TryCatch(async (req, res, next) => {
    const user = req.user;
    if (!user) {
        throw new ErrorHandler("Authentication is Required", 400);
    }
...
export const AddSkillsToUserController = TryCatch(async (req, res, next) => {
    ...
    const result = await AddSkillToUser(userId, skill.trim());
    return res.status(200).json({
        success: true,
...
export const DeleteSkillFromUserController = TryCatch(async (req, res, next) => {
    const user = req.user;
    const { skill } = req.body;
    if (!user) {
        throw new ErrorHandler("Unauthorized", 401);
    ...
    if (skillResult.length === 0) {
        throw new ErrorHandler(`${skill} skill does not exist`, 404);
    }
    ...
    if (userSkill.length === 0) {
        throw new ErrorHandler(`User does not have ${skill} skill`, 400);
    }
    // Delete skill
    await sql `
