...
export const GetCompleteUserDetailbyUserId = async (id) => {
    const sql = await neon(DB_URL);
...
export const UpdateProfile = async (userid, name, PhoneNo, bio) => {
    const sql = await neon(DB_URL);
...
export const UpdateProfilePicture = async (id, url, public_id) => {
    const sql = await neon(DB_URL);
    const user = await sql `
    UPDATE users 
...
export const UpdateResume = async (id, url, public_id) => {
    const sql = await neon(DB_URL);
    const user = await sql `
    UPDATE users 
    SET resume=${url},
...
export const RemoveSkillFromUser = async (userId, skillName) => {
    const sql = await neon(DB_URL);
    ...
    `;
    if (skill.length === 0) {
        throw new ErrorHandler(`${skillName} skill does not exist`, 400);
    }
    ...
    if (userSkill.length === 0) {
        throw new ErrorHandler(`User does not have ${skillName} skill`, 400);
    }
    // Delete skill
    await sql `
